...
TARGET_AGL_M="${RADIAL_TARGET_AGL_M:-1000}"
LATITUDE="${RADIAL_LATITUDE:-43.770833}"
LONGITUDE="${RADIAL_LONGITUDE:-7.419722}"
SAINTE_BAUME_LATITUDE="${RADIAL_SAINTE_BAUME_LATITUDE:-43.312222}"
SAINTE_BAUME_LONGITUDE="${RADIAL_SAINTE_BAUME_LONGITUDE:-5.689722}"
RANGE_M="${RADIAL_RANGE_M:-400000}"
RESOLUTION_M="${RADIAL_RESOLUTION_M:-90}"
TIMEOUT_SECONDS="${RADIAL_TIMEOUT_SECONDS:-900}"
TILE_CONCURRENCY="${RADIAL_TILE_CONCURRENCY:-8}"
RADAR_ID="${RADIAL_RADAR_ID:-11111111-1111-4111-8111-111111111111}"
SAINTE_BAUME_RADAR_ID="${RADIAL_SAINTE_BAUME_RADAR_ID:-22222222-2222-4222-8222-222222222222}"
...
radar_json=$(jq -n --arg id "$RADAR_ID" --argjson lat "$LATITUDE" --argjson lon "$LONGITUDE" --argjson range "$RANGE_M" '{id:$id,name:"Mont Agel",latitude:$lat,longitude:$lon,antenna_agl_m:20,range_m:$range,active:true}')
sainte_baume_json=$(jq -n --arg id "$SAINTE_BAUME_RADAR_ID" --argjson lat "$SAINTE_BAUME_LATITUDE" --argjson lon "$SAINTE_BAUME_LONGITUDE" --argjson range "$RANGE_M" '{id:$id,name:"Pic de Bertagne (Sainte-Baume)",latitude:$lat,longitude:$lon,antenna_agl_m:20,range_m:$range,active:true}')
echo "[2/7] Création/mise à jour des radars"
post_json "$API_URL/api/v1/radars" "$radar_json" "$OUTPUT_DIR/radar.json"
post_json "$API_URL/api/v1/radars" "$sainte_baume_json" "$OUTPUT_DIR/radar-sainte-baume.json"

job_json=$(jq -n --argjson radar "$radar_json" --argjson sainte "$sainte_baume_json" --argjson resolution "$RESOLUTION_M" '{radars:[$radar,$sainte],resolution_m:$resolution,effective_earth_k:1.3333333333333333,target_heights_agl_m:[30,50,100]}')
...
fusion_json=$(jq -n --arg id "$RADAR_ID" --arg sainte "$SAINTE_BAUME_RADAR_ID" --argjson height "$TARGET_AGL_M" '{radar_ids:[$id,$sainte],target_agl_m:$height}')
...
extent_min_x=$(jq -er '.metadata.extent[0]' "$OUTPUT_DIR/metadata.json")
extent_max_y=$(jq -er '.metadata.extent[3]' "$OUTPUT_DIR/metadata.json")
...
<body><header class="toolbar"><span class="brand">RADIAL WMTS</span><label>Couche <select id="layer"></select></label><label>LOD <select id="lod"></select></label><button id="minus" title="Zoom arrière">−</button><button id="plus" title="Zoom avant">+</button><button id="fit">Ajuster</button><label><input id="grid" type="checkbox" checked> Grille des tuiles</label><strong>Mont Agel + Sainte-Baume</strong><span class="status" id="status"></span></header><main id="viewport"><canvas id="map"></canvas><div class="help"><span id="legend"></span> · Molette : zoom · Glisser : déplacer · [ / ] : changer de LOD · croix : radars</div><div class="tooltip" id="tooltip"></div></main>
<script>
const layers='${layers[*]}'.split(' '), matrices=[${matrix_specs%,}], sourceWidth=${width}, sourceHeight=${height}, sourceResolution=${RESOLUTION_M}, extentMinX=${extent_min_x}, extentMaxY=${extent_max_y}, sampleEndpoint='${base}/sample', sampleUrl='/__radial_sample', totalTiles=${tile_count};
const radarDefinitions=[{name:'Mont Agel',lat:${LATITUDE},lon:${LONGITUDE},color:'#ff3b30'},{name:'Pic de Bertagne',lat:${SAINTE_BAUME_LATITUDE},lon:${SAINTE_BAUME_LONGITUDE},color:'#ffcc00'}],projectionCenterLat=radarDefinitions.reduce((sum,r)=>sum+r.lat,0)/radarDefinitions.length,projectionCenterLon=Math.atan2(radarDefinitions.reduce((sum,r)=>sum+Math.sin(r.lon*Math.PI/180),0),radarDefinitions.reduce((sum,r)=>sum+Math.cos(r.lon*Math.PI/180),0))*180/Math.PI;
function project(lat,lon){const earth=6371000,lat0=projectionCenterLat*Math.PI/180,lon0=projectionCenterLon*Math.PI/180,phi=lat*Math.PI/180,d=((lon*Math.PI/180-lon0+Math.PI)%(2*Math.PI)+2*Math.PI)%(2*Math.PI)-Math.PI,c=Math.acos(Math.max(-1,Math.min(1,Math.sin(lat0)*Math.sin(phi)+Math.cos(lat0)*Math.cos(phi)*Math.cos(d)))),k=Math.abs(c)<1e-14?1:c/Math.sin(c),x=earth*k*Math.cos(phi)*Math.sin(d),y=earth*k*(Math.cos(lat0)*Math.sin(phi)-Math.sin(lat0)*Math.cos(phi)*Math.cos(d));return {x,y}}
const radars=radarDefinitions.map(r=>{const p=project(r.lat,r.lon);return {...r,col:(p.x-extentMinX)/sourceResolution,row:(extentMaxY-p.y)/sourceResolution}});
const canvas=document.getElementById('map'),ctx=canvas.getContext('2d'),layerSelect=document.getElementById('layer'),lodSelect=document.getElementById('lod'),status=document.getElementById('status'),tooltip=document.getElementById('tooltip'),legend=document.getElementById('legend'),images=new Map(),samples=new Map();let sampleTimer=0,sampleController=null;
const targetLayer='agl-${TARGET_AGL_M}m';let lod=matrices.length-1,layer=layers.includes(targetLayer)?targetLayer:layers[0],zoom=1,offsetX=0,offsetY=0,drag=null;
for(const name of layers)layerSelect.add(new Option(name,name));layerSelect.value=layer;for(const matrix of matrices)lodSelect.add(new Option('LOD '+matrix[0]+' · '+matrix[2]+'×'+matrix[3]+' · facteur '+matrix[1],matrix[0]));lodSelect.value=lod;
...
function selectedAgl(){if(layer==='ground')return 0;const match=/^agl-(\d+)m$/.exec(layer);return match?Number(match[1]):null}
function updateLegend(){const agl=selectedAgl();legend.textContent=agl===null?(layer==='min-detection-height'?'Hauteur minimale : clair = faible, noir = NoData':'Nombre de radars'):'Détection à '+agl+' m AGL : blanc = VU, noir = NON VU'}
function sampleText(value){const terrainValue=value.terrain_elevation_amsl_m,minimumAgl=value.minimum_detection_agl_m,minimumAmsl=value.minimum_detection_amsl_m,terrain=terrainValue===null?'NoData':terrainValue+' m AMSL',minimum=minimumAmsl===null?'NoData':minimumAmsl+' m AMSL',agl=selectedAgl();if(agl===null)return ' · Terrain '+terrain+' · Détection min. '+minimum;const seen=minimumAgl!==null&&agl>=minimumAgl,targetAmsl=terrainValue===null?'NoData':(terrainValue+agl)+' m AMSL';return ' · '+(seen?'VU':'NON VU')+' à '+agl+' m AGL · Cible '+targetAmsl+' · Terrain '+terrain+' · Détection min. '+minimum}
function updateTooltip(event){const rect=canvas.getBoundingClientRect(),x=event.clientX-rect.left,y=event.clientY-rect.top,m=matrix(),sourceCol=(x-offsetX)/zoom*m[1],sourceRow=(y-offsetY)/zoom*m[1];if(sourceCol<0||sourceRow<0||sourceCol>=sourceWidth||sourceRow>=sourceHeight){tooltip.style.display='none';return}const col=Math.floor(sourceCol),row=Math.floor(sourceRow),key=col+','+row,nearest=radars.map(r=>{const east=(sourceCol-r.col)*sourceResolution,north=(r.row-sourceRow)*sourceResolution;return {radar:r,distance:Math.hypot(east,north)/1000,bearing:(Math.atan2(east,north)*180/Math.PI+360)%360}}).sort((a,b)=>a.distance-b.distance)[0],baseText=nearest.radar.name+' · Cap '+nearest.bearing.toFixed(1)+'° · '+nearest.distance.toFixed(2)+' km';tooltip.dataset.sampleKey=key;tooltip.textContent=baseText+(samples.has(key)?sampleText(samples.get(key)):' · altitudes…');tooltip.style.left=x+'px';tooltip.style.top=y+'px';tooltip.style.display='block';clearTimeout(sampleTimer);sampleTimer=setTimeout(()=>{if(sampleController)sampleController.abort();sampleController=new AbortController();fetch(sampleUrl+'?col='+col+'&row='+row,{signal:sampleController.signal}).then(response=>{if(!response.ok)throw new Error('HTTP '+response.status);return response.json()}).then(value=>{samples.set(key,value);if(tooltip.dataset.sampleKey===key)tooltip.textContent=baseText+sampleText(value)}).catch(error=>{if(error.name!=='AbortError'&&tooltip.dataset.sampleKey===key)tooltip.textContent=baseText+' · altitudes indisponibles ('+error.message+')'})},80)}
...
layerSelect.onchange=()=>{layer=layerSelect.value;updateLegend();draw()};lodSelect.onchange=()=>{lod=Number(lodSelect.value);fit()};document.getElementById('grid').onchange=draw;document.getElementById('minus').onclick=()=>zoomAt(.5,canvas.clientWidth/2,canvas.clientHeight/2);document.getElementById('plus').onclick=()=>zoomAt(2,canvas.clientWidth/2,canvas.clientHeight/2);document.getElementById('fit').onclick=fit;addEventListener('keydown',event=>{if(event.key==='['&&lod>0){lod--;lodSelect.value=lod;fit()}if(event.key===']'&&lod<matrices.length-1){lod++;lodSelect.value=lod;fit()}});new ResizeObserver(resize).observe(document.getElementById('viewport'));updateLegend();resize();fit();
</script></body></html>
HTML
