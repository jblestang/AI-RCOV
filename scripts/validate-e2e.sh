...
function sampleText(value){const terrainValue=value.terrain_elevation_amsl_m,minimumValue=value.minimum_detection_agl_m,terrain=terrainValue===null?'NoData':terrainValue+' m AMSL',minimum=minimumValue===null?'NoData':minimumValue+' m AGL',agl=selectedAgl();if(agl===null)return ' · Terrain '+terrain+' · Détection min. '+minimum;const seen=minimumValue!==null&&agl>=minimumValue,targetAmsl=terrainValue===null?'NoData':(terrainValue+agl)+' m AMSL';return ' · '+(seen?'VU':'NON VU')+' à '+agl+' m AGL · Cible '+targetAmsl+' · Terrain '+terrain+' · Détection min. '+minimum}
