//! CLI tool and benchmark runner for hierarchical H3 radar coverage export.

use coverage_h3::{export_rhgt_file_to_h3_jsonl, extract_h3_records, H3ExportConfig};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if bench_mode {
        run_benchmark(config, bench_cell_m, bench_range_m)?;
    } else {
        let input = input_path.ok_or("missing input .rhgt file (or pass --bench)")?;
...
fn run_benchmark(
    ...
    let radius = (range_m / cell_m).ceil() as usize;
    ...
    let half = (dimension as f64 * cell_m) / 2.0;
    ...
    for r in 0..dimension {
        let dy = (r as isize - radius as isize) as f64 * cell_m;
        for c in 0..dimension {
            let dx = (c as isize - radius as isize) as f64 * cell_m;
            let dist = dx.hypot(dy);
            if dist <= range_m {
                // Simulate terrain shadow: blind spot behind a ridge at azimuth 45° to 90°
